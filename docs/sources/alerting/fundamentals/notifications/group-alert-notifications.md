...
By default, notification policies in Grafana group alerts by the alert rule. Specifically, they are grouped using the `alertname` and `IESG_folder` labels, as alert rule names are not unique across folders.
